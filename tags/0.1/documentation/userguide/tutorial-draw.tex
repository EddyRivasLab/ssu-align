...
SSU sequences using the \prog{--indi} option:

\user{ssu-draw --indi myseqs}

As before, this command will create the nine-page alignment summary
diagram files, but it will also create \prog{indi.pdf} suffixed
files which display each sequence in the alignment. 

Take a look at \prog{myseqs.archaea.indi.pdf} or
\prog{myseqs.archaea.indi.ps}. This file has 10 pages, two per
